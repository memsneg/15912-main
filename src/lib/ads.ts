...
const AREA_SEO_VARIANTS: ((brand: string, area: string, gov?: string) => string)[] = [
  ...
  (brand: string, area: string) =>
    `خدمات صيانة ${brand} في ${area} متاحة طوال أيام الأسبوع. فريقنا يعمل على إصلاح جميع أجهزة ${brand} بكفاءة عالية وأسعار مناسبة. نوفر ضماناً على الصيانة واستبدال القطع بضمان الجودة. تواصل معنا عبر الخط الساخن  للحجز الفوري.`,
  (brand: string, area: string) =>
  ...
  (brand: string, area: string) =>
    `تغطي خدمات صيانة ${brand} في ${area} جميع احتياجاتك من إصلاح الأجهزة المنزلية. فنيونا المدربون يحملون قطع الغيار الأصلية لإتمام الصيانة في أقل وقت. اتصل الآن  للحجز المباشر.`,
  (brand: string, area: string) =>
